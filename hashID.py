...
def writeResult(identified_modes, hashcatMode=False, johnFormat=False, extended=False):
    ...
    for mode in identified_modes:
        if extended:
            count += 1
            hashData = []
            if mode.name == "BigCrypt" or mode.name == "Cisco Type 7":
                continue
            hashData.append(mode.name)
            if mode.hashcat is not None:
                hashData.append(mode.hashcat)
            if mode.john is not None:
                hashData.append(mode.john)
            hashTypes.append(hashData)
    return hashTypes
...
def main(string):
    ...
    hashID = HashID()
    return writeResult(hashID.identifyHash(string),extended = True)
